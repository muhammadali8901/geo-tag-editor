(function() {
    ...
    function cacheEls() {
        ["dropZone", "fileInput", "previewArea", "removeBtn", "metaBody", "latInput", "lngInput", "applyBtn", "removeGpsBtn", "downloadBtn", "resultBanner", "resultText", "mapContainer", "tabUpload", "tabEdit", "tabDownload", "editSection", "downloadSection", "uploadSection", "imagesPreviewContainer", "processedImagesContainer", "uploadedImagesPreview", "addMoreBtn", "getCurrentLocBtn", "locBtnText"].forEach(function(id) {
            els[id] = $(id)
        })
    }
    ...
    function decimalToDMS(decimal) {
        ...
        const seconds = Math.round((minutesFloat - minutes) * 60 * 1e4);
        return [
            [degrees, 1],
            [minutes, 1],
            [seconds, 1e4]
    ...
    function ensurePiexif() {
        ...
        if (!piexifPromise) { piexifPromise = loadScript(PIEXIF_URL, function() { return !!window.piexif }) }
    ...
    function ensureJSZip() {
        ...
        if (!jszipPromise) { jszipPromise = loadScript(JSZIP_URL, function() { return !!window.JSZip }) }
        return jszipPromise
    }
    ...
    function initMapAsync(lat, lng) {
        ...
        ensureLeaflet().then(function() { initMap(lat, lng) }).catch(function() { renderMapStatus("Map failed to load. You can still edit coordinates manually.", true) })
    }
    ...
    function handleFiles(files) {
        ...
        const newFiles = Array.from(files).filter(function(file) { return file.type.match(/image\/jpe?g/i) });
            ...
            filesToAdd.forEach(function(file) {
                ...
                reader.onload = function() {
                    ...
                    img.onload = function() {
                        ...
                        loadedInBatch++;
                        if (loadedInBatch === filesToAdd.length) {
                            updateUploadPreview();
                            readExifFromFirstImage();
                            setTab("edit");
                            GTP.showToast(filesToAdd.length + " image(s) processed.", "success");
                        }
                    };
                    img.onerror = function() {
                        loadedInBatch++;
                        GTP.showToast("Failed to load image: " + file.name, "error");
                        if (loadedInBatch === filesToAdd.length) {
                            updateUploadPreview();
                ...
                reader.onerror = function() {
                    loadedInBatch++;
                    GTP.showToast("Failed to read file: " + file.name, "error");
                    if (loadedInBatch === filesToAdd.length) {
                        updateUploadPreview();
                    }
                };
    ...
    function updateUploadPreview() {
        ...
        statusHTML += uploadedImages.map(function(img, idx) {
            return '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px;background:var(--bg);border-radius:var(--radius-sm);margin-bottom:8px">' +
                ...
                '</div>' +
                '<button class="remove-single-img" data-index="' + idx + '" style="background:none;border:none;color:var(--danger);cursor:pointer;padding:4px;" title="Remove image">' +
                '<svg class="icon" style="width:16px;height:16px;"><use href="/images/icons.svg#icon-close"></use></svg></button>' +
                '</div>'
        }).join("");
        if (uploadedImages.length >= MAX_IMAGES) {
        ...
        // Attach individual remove listeners
        els.uploadedImagesPreview.querySelectorAll(".remove-single-img").forEach(btn => {
            btn.addEventListener("click", function(e) {
                e.stopPropagation();
                const idx = parseInt(this.getAttribute("data-index"));
                uploadedImages.splice(idx, 1);
                updateUploadPreview();
                if (uploadedImages.length > 0) readExifFromFirstImage();
                else resetTool();
            });
        });
    ...
    function updateImagesPreview() {
        ...
        els.imagesPreviewContainer.innerHTML = uploadedImages.map(function(img, idx) {
            return '<div style="margin-bottom:20px">' + '<div class="preview-img-wrap">' + '<img src="' + img.dataURL + '" alt="Image ' + (idx + 1) + ' preview" style="max-height:250px" onerror="console.log(\'Image load error\'); this.style.display=\'none\'">' + "</div>" + '<div style="margin-top:8px">' + '<strong style="font-size:.9rem">' + img.name + "</strong>" + '<span style="font-size:.82rem;color:var(--text-muted);margin-left:8px">' + formatBytes(img.size) + "</span>" + "</div>" + "</div>"
        }).join("")
    }
    ...
    function readExifFromFirstImage() {
        ...
        } catch (_) { rows.push(["EXIF Data", "No EXIF metadata found"]) }
        if (!rows.length) rows.push(["EXIF Data", "No EXIF metadata found"]);
        els.metaBody.innerHTML = rows.map(function(row) { return '<tr><th scope="row">' + row[0] + "</th><td>" + row[1] + "</td></tr>" }).join("");
        ...
        setTimeout(function() {
            try {
                uploadedImages.forEach(function(img) {
                    let exif;
                    try { exif = piexif.load(img.dataURL) } catch (_) { exif = { "0th": {}, Exif: {}, GPS: {}, Interop: {}, "1st": {} } }
    ...
    function displayProcessedImages() {
        ...
        els.processedImagesContainer.innerHTML = '<h3 style="font-size:1.1rem;font-weight:600;margin-bottom:16px">Processed Images</h3>' + uploadedImages.map(function(img, idx) {
            const dataURL = img.modifiedDataURL || img.dataURL;
            return '<div style="margin-bottom:24px;padding:20px;background:var(--bg);border-radius:var(--radius);border:1px solid var(--border)">' + '<div class="preview-img-wrap" style="margin-bottom:12px">' + '<img src="' + dataURL + '" alt="Processed image ' + (idx + 1) + '" style="max-height:250px">' + "</div>" + '<div style="display:flex;justify-content:space-between;align-items:center">' + '<strong style="font-size:.9rem">' + img.name + "</strong>" + '<button class="btn btn-success btn-sm download-single-btn" data-index="' + idx + '">' + '<svg class="icon"><use href="/images/icons.svg#icon-download"></use></svg> Download Processed Image' + "</button>" + "</div>" + "</div>"
        }).join("");
        document.querySelectorAll(".download-single-btn").forEach(function(btn) { btn.addEventListener("click", function() { const idx = parseInt(this.getAttribute("data-index")); downloadSingleImage(idx) }) });
        if (els.downloadBtn) { if (uploadedImages.length > 1) { els.downloadBtn.innerHTML = '<svg class="icon"><use href="/images/icons.svg#icon-download"></use></svg> Download All Processed Images' } else { els.downloadBtn.innerHTML = '<svg class="icon"><use href="/images/icons.svg#icon-download"></use></svg> Download Processed Image' } }
    }
    ...
    function removeGPS() {
        ...
        try {
            uploadedImages.forEach(function(img) {
                let exif;
                try { exif = piexif.load(img.dataURL) } catch (_) { return }
                exif.GPS = {};
                img.modifiedDataURL = piexif.insert(piexif.dump(exif), img.dataURL);
                img.action = "cleaned"
            });
        ...
        } catch (_) { showResult("Error removing GPS metadata. Please try another image.", "error") }
    }

    function resetTool() {
        uploadedImages = [];
        if (els.fileInput) els.fileInput.value = "";
        if (els.latInput) els.latInput.value = "";
        if (els.lngInput) els.lngInput.value = "";
        if (els.metaBody) els.metaBody.innerHTML = "";
        if (els.processedImagesContainer) {
    ...
    function init() {
        ...
        els.fileInput.addEventListener("change", function() { handleFiles(this.files) });
        ...
        if (els.getCurrentLocBtn) {
            els.getCurrentLocBtn.addEventListener("click", function() {
                ...
                navigator.geolocation.getCurrentPosition(function(position) {
                    ...
                    validateCoords();
                    updateMapMarker();
                    setTimeout(function() { if (els.locBtnText.textContent === "Location Fetched Successfully!") { els.locBtnText.textContent = "📍 Get My Current Location" } }, 5e3)
                }, function(error) {
                    els.getCurrentLocBtn.classList.remove("loading");
        ...
        els.tabUpload.addEventListener("click", function() { setTab("upload") });
        ...
        els.downloadBtn.addEventListener("click", function() {
            ...
            ensureJSZip().then(function() {
                ...
                zip.generateAsync({ type: "blob" }).then(function(content) {
                    ...
                    hideResult()
                }).catch(function(err) { showResult("Error creating ZIP file.", "error") })
            }).catch(function() {
                showResult("Error loading JSZip. Downloading individually...", "warning");
                uploadedImages.forEach(function(_, idx) { setTimeout(function() { downloadSingleImage(idx) }, idx * 500) })
            })
        });
        setTab("upload")
    }
